...
import scalation.modeling.{SymRidgeRegression, Regression}
import scalation.modeling.qk


@main def SymbolicRidgeRegression1(): Unit =
// //   println("Hello, World!")
...
//     // %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
//     val mod = SymRidgeRegression (x, y, null, scala.collection.mutable.Set(-2.0, -1.0, 0.5, 2.0))    // add cross-terms and given powers
...
//     end for
    ...
    val mod = SymRidgeRegression (x, y, ox_fname, scala.collection.mutable.Set(-2.0, -1.0, 0.5, 2.0))
    mod.trainNtest ()()
    println (mod.summary ())                                  // parameter/coefficient statistics
    ...
    for (c, r) <- imp1 do println (s"col = $c, \t x_${c}, \t importance = $r") 
    ...
    for (c, r) <- imp2 do println (s"col = $c, \t x_${c}, \t importance = $r") 
    ...
    for (c, r) <- imp3 do println (s"col = $c, \t x_${c}, \t importance = $r") 
