...
@main def NeuralNetworkXLAutoMPG(): Unit =
    ...
    // var x = MatrixD(xRows.map(row => VectorD(row)).toIndexedSeq)
    ...
    // keep as MatrixD (n x 1) if your pipeline expects a column matrix
    // val y = new MatrixD(yv.dim, 1)
    // for (i <- 0 until yv.dim) y(i, 0) = yScaledV(i)
