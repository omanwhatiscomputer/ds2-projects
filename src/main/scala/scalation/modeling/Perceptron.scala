...
//  val (cols, rSq) = mod.backwardElimAll ()                        // R^2, R^2 bar, R^2 cv
    ...
    new PlotM (null, rSq.transpose, Regression.metrics, "R^2 vs n for Perceptron", lines = true)
    println (s"rSq = $rSq")
...
//  println (mod.summary ())                                        // parameter/coefficient statistics
    ...
    for tech <- SelectionTech.values do
        ...
        val k = cols.size
        println (s"k = $k, n = ${x.dim2}")
        new PlotM (null, rSq.transpose, Regression.metrics, s"R^2 vs n for Perceptron with $tech", lines = true)
