...
object NeuralNet_XL4TS:
     ...
     */
    def apply (y: VectorD, lags: Int, h: Int, @unused nz: Int = -1,               // FIX
               hparam: HyperParameter = Optimizer.hp,
               f: Array [AFF] = Array (f_eLU, f_eLU, f_tanh)): NeuralNet_XL =
        val bakcast = false
        val (xy, _) = ARX.buildMatrix (null, y, hparam, bakcast)
        val yy      = makeMatrix4Y (y, h, bakcast)
     ...
     *  @param f       the array of activation function family for layers k->k+1
     */
    def exo (y: VectorD, lags: Int, ex: MatrixD, h: Int, @unused nz: Int = -1,      // FIX
             hparam: HyperParameter = Optimizer.hp,
             f: Array [AFF] = Array (f_eLU, f_eLU, f_tanh)): NeuralNet_XL =
        val bakcast = false
        val (xy, _) = ARX.buildMatrix (ex, y, hparam, bakcast)
        val yy      = makeMatrix4Y (y, h, bakcast)
...
//  val (yp, qof) = mod.trainNtest2 (x_, y_)(xx, yy)                            // train on (x_, y_) and test on (xx, yy)
    ...
    val yp = mod.trainNtest2 ()()._1                                            // train on full and test on full
...
/*
    ...
    new PlotM (null, rSq.transpose, Regression.metrics, s"R^2 vs n for NeuralNet_XL4TS with tech", lines = true)
//  println (mod.summary ())
...
//  val f_ : Array [AFF] = Array (f_eLU, f_eLU, f_tanh)
    ...
    banner ("In-ST: Test NeuralNet_XL4TS on COVID-19 Weekly Data")
    val mod = NeuralNet_XL4TS.exo (y, LAGS, x, h, f = f_)                       // create model for time series data
