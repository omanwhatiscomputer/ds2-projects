...
// FIX -- use hyper-parameters
    ...
    def backward (x: TensorD, y: TensorD, batch_start: Int, batch_end: Int,
        ...
        for t <- seq_length - 1 to truncated_start by -1 do                 // Iterate backwards through the sequence
            ...
            V.grad   += dyp * H.param(?, ?, t).𝐓                            // Update the gradient of the output weights
            H.grad(?, ?, t) = (V.param.𝐓 * dyp) + dh_next                   // Calculate the gradient of the hidden state
            ...
            U.grad += H.pre_act_grad * x(t).𝐓                               // Update the gradient of the input weights
            W.grad += H.pre_act_grad * H_prev.𝐓                             // Update the gradient of the hidden weights
            dh_next = W.param.𝐓 * H.pre_act_grad                            // Calculate the gradient of the next hidden state
        end for
    end backward
