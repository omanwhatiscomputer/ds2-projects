...
@main def tranARYTest2 (): Unit =
    ...
    mod.diagnoseAll (mod.getY, mod.getYf, Forecaster.teRng (y.dim))     // only diagnose on the testing set - tran
    mod.diagnoseAll (y, mod.getYf.map_ (mod.itran), Forecaster.teRng (y.dim))  // only diagnose on the testing set - orig
