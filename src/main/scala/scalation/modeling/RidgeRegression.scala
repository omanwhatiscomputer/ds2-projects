...
object RidgeRegression:
    ...
    /** Base hyper-parameter specification for `RidgeRegression` and other regularized regression
     */
    ...
    hp += ("eps", 1e-8, 1e-8)                                          // small constant to avoid zero division
    hp += ("pow", 0.5, 0.5)                                            // exponent
...
//  println (s"y = $y")                                                // response vector
    ...
    for tech <- SelectionTech.values do
        ...
        new PlotM (null, rSq.transpose, Regression.metrics, s"R^2 vs n for RidgeRegression with $tech", lines = true)
