...
//  val (cols, rSq) = mod.backwardElimAll ()                  // R^2, R^2 bar, sMAPE, R^2 cv
    ...
    new PlotM (null, rSq.transpose, Regression.metrics, "R^2 vs n for Regression", lines = true)
    println (s"rSq = $rSq")
...
//  val (x, y) = data.toMatrixDD (ArrayBuffer.range (1, 7), 0)
    ...
    new PlotM (t, rSq.transpose, Regression.metrics, "R^2 vs n for Regression", lines = true)
    println (s"rSq = $rSq")
